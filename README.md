...
本模型共有 **5个待辨识参数**，通过优化算法从实验数据中辨识。几何参数（$R_p, R_n$）和最大浓度（$c_{s,\max}$）设为固定值，欧姆内阻 $R_e$ 在测量值范围内微调：

| 序号 | 参数         | 符号    | 辨识范围                    | 单位  | 物理意义                 | 初始猜测值            |
| ---- | ------------ | ------- | --------------------------- | ----- | ------------------------ | --------------------- |
| 1    | 正极扩散系数 | $D_p$ | $10^{-16}$ ~ $10^{-12}$ | m²/s | 正极颗粒内锂离子扩散速率 | $1 \times 10^{-14}$ |
| 2    | 负极扩散系数 | $D_n$ | $10^{-16}$ ~ $10^{-12}$ | m²/s | 负极颗粒内锂离子扩散速率 | $3 \times 10^{-14}$ |
| 3    | 正极反应速率 | $k_p$ | $10^{-13}$ ~ $10^{-8}$  | -     | 正极表面反应动力学常数   | $2 \times 10^{-11}$ |
| 4    | 负极反应速率 | $k_n$ | $10^{-13}$ ~ $10^{-8}$  | -     | 负极表面反应动力学常数   | $2 \times 10^{-11}$ |
| 5    | 欧姆内阻     | $R_e$ | 测量值范围 (0.01~0.2)       | Ω    | 接触电阻与离子电阻       | 0.05                  |
...
| ------------ | ---------------- | ----------------- | ------ |
| 正极颗粒半径 | $R_p$          | $8$ μm         | 典型值 |
| 负极颗粒半径 | $R_n$          | $10$ μm        | 典型值 |
| 正极最大浓度 | $c_{s,\max,p}$ | $51385$ mol/m³ | 典型值 |
| 负极最大浓度 | $c_{s,\max,n}$ | $33133$ mol/m³ | 典型值 |
...
2. **参数演变趋势**：随着电池老化（循环次数增加），辨识参数会呈现规律性变化：
   ...
   - **欧姆内阻** $R_e$：随老化逐渐增大（SEI膜增厚、接触电阻增加）——来自测量值+微调
3. **$R_e$ 辨识策略的原因**：

   - NASA Battery Dataset 提供了每次阻抗测量的 $R_e$ 值（见 metadata.csv），可以作为先验知识确定合理范围。
   - 允许 $R_e$ 在一定范围内微调，可以吸收模型误差和接触电阻的瞬态变化，显著提高电压曲线拟合精度（RMSE < 0.02V）。
   - 这是一种**“测量定界 + 优化微调”**的混合策略。
...
| 符号                       | 含义                     | 状态                   | 说明                                                                 |
| -------------------------- | ------------------------ | ---------------------- | -------------------------------------------------------------------- |
| $\boldsymbol{\theta}$    | 参数向量                 | **待辨识**       | $\boldsymbol{\theta} = [D_p, D_n, k_p, k_n, R_e]^T$（5个核心参数） |
...
输入层: (batch_size, seq_len, 7)     ← 7维特征向量序列
    ↓
物理特征映射: MLP (Linear->ReLU->Linear)  ← 将物理参数映射到高维特征空间
    ↓
...
| --------------------- | ------------------ | -------------------- | ------------------------- |
| **XGBoost**     | 0.0739             | 0.0466               | 显著提升 (树模型鲁棒性好) |
| **GRN**         | 0.1652             | 0.1584               | 略微提升 (两者均表现较差) |
| **Transformer** | 0.0654             | 0.0374               | 显著提升 (物理特征有效)   |
| **LSTM**        | 0.0220             | **0.0136**     | **显著提升 (最优)** |

**结论摘要**：

- **SP-LSTM 表现最优**：通过引入物理特征及双流特征融合架构，RMSE 降至 **0.0136**，显著优于所有对比模型。
- **物理特征的有效性**：
  - 对于 **XGBoost**、**Transformer** 和 **SP-LSTM**，物理特征带来了显著的性能提升。
  - 对于 **GRN**，虽然略有提升，但整体性能较差（RMSE > 0.15），可能由于循环网络结构对该任务的适应性不足或过拟合。
- **公平性设置**：
  - 统一随机种子 (Seed=42)。
...
| $R_e$ | 欧姆内阻         | 0.01 ~ 0.2                  | Ω    | 0.05                  |

> **注**：$R_e$ 的辨识范围由训练电池的实测阻抗值确定，通过在线微调以补偿接触电阻变化。几何参数（$R_p, R_n$）和最大浓度（$c_{s,\max}$）设为固定值。
